-- "ราคาปกติ" ต่อ (แพ็กเกจราคา × ประเภทห้อง) — ไม่ผูกวัน ไม่หมดอายุ (เจ้าของเสนอ 2026-07-16)
--
-- โมเดลเดิมบังคับราคาผูกช่วงวันเสมอ (rate_prices รายวัน) → เกิดช่วงโหว่ "จองไม่ได้"
-- ต้องไล่ตั้งใหม่ทุกปี · โมเดลใหม่: ราคาปกติยืนพื้น + rate_prices กลายเป็น "override"
-- เฉพาะช่วง (high season / วันหยุด / ปิดขาย) → ไม่มีช่วงโหว่อีก
--
-- ลำดับราคาใน create_booking: override รายวัน > ราคาปกติ > error 'ยังไม่ตั้งราคา'
-- closed ใน override = ปิดขายวันนั้นชัดเจน (ไม่ fallback)

create table public.rate_base_prices (
...
create index rate_base_prices_hotel_idx on public.rate_base_prices (hotel_id);

alter table public.rate_base_prices enable row level security;

-- policy triplet (pattern เดียวกับ rate_prices)
create policy rate_base_prices_select on public.rate_base_prices for select to authenticated
  using (public.user_role_in_hotel(hotel_id) is not null or public.is_super_admin());
create policy rate_base_prices_write on public.rate_base_prices for all to authenticated
...
-- create_booking: เพิ่ม fallback ราคาปกติ (แก้จากเวอร์ชัน migration 21)
...
begin
  ...
  while v_day < p_check_out loop
    ...
    -- ราคาวันนี้: override รายวัน (rate_prices) มาก่อน · closed = ปิดขายวันนั้น
    -- ไม่มี override → ใช้ "ราคาปกติ" (rate_base_prices — ไม่ผูกวัน ไม่หมดอายุ)
