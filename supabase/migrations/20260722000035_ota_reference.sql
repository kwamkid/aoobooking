-- ============================================================================
-- เลขอ้างอิงจาก OTA (เจ้าของถาม "จองจาก OTA มี 2 เลขมั้ย" — ใช่: เลขเรา BK-XXXXXX
-- + เลขของ OTA) — เก็บแยก `bookings.ota_reference` · Phase 4 channel manager
-- เป็นคน populate (ตอนนี้ null สำหรับจองตรง) · + search_bookings v4:
-- เพิ่ม guest_id (ชื่อแขกในตาราง → ลิงก์โปรไฟล์) + ota_reference
-- ============================================================================

alter table public.bookings add column if not exists ota_reference text;

-- return type เปลี่ยน → drop ก่อน (pattern เดิม migration 27/34)
